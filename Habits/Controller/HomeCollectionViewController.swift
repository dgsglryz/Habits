...
class HomeCollectionViewController: UICollectionViewController {
  ...
  enum ViewModel {
    ...
    enum Item: Hashable {
      case leaderboardHabit(name: String, leadingUserRanking: String?, secoundaryUserRanking: String?)
  ...
  func createDataSource() -> DataSourceType {
    let dataSource = DataSourceType(collectionView: collectionView) { (collectionView, indexPath, item) -> UICollectionViewCell? in
      switch item{
      case .leaderboardHabit(let name, let leadingUserRanking, let secoundaryUserRanking):
        ...
        cell.secondaryLabel.text = secoundaryUserRanking
  ...
  func createLayout() -> UICollectionViewCompositionalLayout {
    let layout = UICollectionViewCompositionalLayout{ (sectionIndex, enviroment) -> NSCollectionLayoutSection? in
      ...
      case .leaderboard:
        let leaderboaredItemSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1), heightDimension: .fractionalHeight(0.3))
        let leaderboardItem = NSCollectionLayoutItem(layoutSize: leaderboaredItemSize)
        
        let verticalTrioSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(0.75), heightDimension: .fractionalWidth(0.75))
        let leaderboaredVerticalTrio = NSCollectionLayoutGroup.vertical(layoutSize: verticalTrioSize, subitem: leaderboardItem, count: 3)
        leaderboaredVerticalTrio.interItemSpacing = .fixed(10)
        
        let leaderboardSection = NSCollectionLayoutSection(group: leaderboaredVerticalTrio)
  ...
  func update(){
    ...
    combinedStatisticsRequestTask = Task{
      if let combinedStatistics = try? await CombineStatisticsRequest().send(){
  ...
  func updateCollectionView(){
    ...
    let leaderboardItems = model.habitStatistics.filter { statistic in
      ...
      .reduce(into: [ViewModel.Item]()) { partial, statistic in
        ...
        default:
          ...
          if let myContIndex = myCountIndex, myCountIndex != rankedUserCounts.startIndex {
            secondaryRanking = userRankingString(from: rankedUserCounts[myContIndex])
          } else {
            secondaryRanking = userRankingString(from: rankedUserCounts[1])
          }
        }
        let leaderboardItem = ViewModel.Item.leaderboardHabit(name: statistic.habit.name, leadingUserRanking: leadingRanking, secoundaryUserRanking: secondaryRanking)
    ...
    for followedUser in model.followedUsers.sorted(by: { $0.name < $1.name}) {
      let message: String
      let followedUserLogggedHabits = loggedHabitNames(for: followedUser)
      
      let commonLoggedHabits = followedUserLogggedHabits.intersection(currentUserLoggedHabits)
      
      if commonLoggedHabits.count > 0 {
        let habitName: String
        let commmonFavoriteLoggedHabits = favoriteLoggedHabits.intersection(commonLoggedHabits)
        
        if commmonFavoriteLoggedHabits.count > 0 {
          habitName = commmonFavoriteLoggedHabits.sorted().first!
        }else{
      ...
      } else if followedUserLogggedHabits.count > 0 {
        let habitName = followedUserLogggedHabits.sorted().first!
        let habitStats = model.habitStatistics.first{$0.habit.name == habitName}!
        ...
        message = "This user doesn't seem to have done much yet. Check in to see if they need any help gettig started."
      }
