...
class UserDetailViewController: UIViewController {
  ...
  func update(){
    ...
    habitLeadStatisticsRequestTask = Task{
      if let userStas = try? await HabitLeadStatisticsRequest(userID: user.id).send(){
        self.model.leadingStats = userStas
      }else{
  ...
  func updateCollectionView(){
    ...
    var itemBySection = userStatistics.habitCounts.reduce(into: [ViewModel.Section: [ViewModel.Item]]()) { partial, habitCount in
